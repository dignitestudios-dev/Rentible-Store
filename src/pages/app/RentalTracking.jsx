...
const RentalTracking = () => {
  ...
  const getRentals = async (query = "") => {
    try {
      ...
      const { data } = await axios.get(
        `/booking?type=rental&search=${query}${
          initialDate
  ...
  return (
    <div className="w-full   flex flex-col  py-4 px-2 lg:px-6 justify-start items-start gap-6">
      <div className="w-full h-auto flex flex-col gap-3 justify-start items-start">
        <div className="w-full relative flex flex-col lg:flex-row justify-between lg:items-center">
          ...
          <div className="w-auto flex justify-start items-center gap-3">
            <div className="relative w-[320px] lg:w-[256px] bg-white h-[49px] flex items-start justify-start rounded-[8px] border border-gray-300">
      ...
      <div className="w-full overflow-x-auto lg:overflow-x-hidden flex flex-col justify-start items-start gap-6">
        <div className="lg:min-w-[805px] min-w-[875px]  flex flex-wrap lg:flex-nowrap gap-2 lg:gap-0   px-[2px] py-[2px] border border-gray-300 bg-white h-[49px] rounded-[8px]">
          {statusLoading
            ? [...Array(8)].map((_, index) => (
                <div
                  key={index}
                  className={`min-w-[90px] h-[45px]  bg-gray-50 rounded-md animate-pulse ${
                    index === 0 ? "rounded-[7px] lg:rounded-l-[7px]" : ""
